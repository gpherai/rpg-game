...
├─ scene.py          # Scene baseclass + SceneManager
├─ resources.py      # ResourceManager (sprites, audio, fonts)
├─ events.py         # Eenvoudige event/signals (optioneel)
...
├─ quests.py         # QuestSystem, quest-state-machine
├─ dialogue.py       # DialogueSystem, graph-evaluatie, conditions/effects
├─ time.py           # TimeSystem, dag/nacht, seizoenen
├─ items.py          # ItemsSystem, inventory, itemeffecten
├─ economy.py        # EconomySystem, shops, currency
├─ progression.py    # ProgressionSystem, XP/level-ups, Tri-profielen
├─ save.py           # SaveSystem, SaveData build/load, trilogie-export (later)
└─ state.py          # Globale flags, choice-history, karma (optioneel separaat)
```

- Elk systeem is zoveel mogelijk **Pygame-onafhankelijk**.  
- `state.py` kan een lichte container zijn voor globale flags/wereldstate, of geïntegreerd in `save.py`.
...
### 2.5 `src/tri_sarira_rpg/presentation/`
...
├─ pause_menu.py     # Pause/menu overlays (later)
└─ ui/
   ├─ __init__.py
   ├─ widgets.py     # Buttons, panels, lists
   ├─ dialogue_box.py# Dialogue UI
   ├─ hud.py         # Overworld HUD (time, minimap, etc. later)
   └─ menus.py       # Inventory, questlog, settings (later)
```

- Scenes liggen hier (in lijn met 5.2).  
- UI-componenten in `presentation/ui/` worden door meerdere scenes gedeeld.

### 2.6 `src/tri_sarira_rpg/utils/`
...
- `default.toml` – standaardinstellingen (resolutie, fullscreen, volume, keybinds).  
...
- `config/default.toml` + `config/dev.toml` (basic).
