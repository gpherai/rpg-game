...
**Status:** Deels geïmplementeerd. Minimale schema's beschikbaar voor: actors, enemies, skills, items, zones, quests, dialogue, npc_schedules, shops.
...
- `TimeBand`:
  - `"dawn" | "day" | "dusk" | "night"` (lowercase in data)  
- `Rarity`:  
...
- Growth weights (level-up):
  - `growth_weights`: object
    - `phys_weight`: number
    - `ment_weight`: number
    - `spir_weight`: number
