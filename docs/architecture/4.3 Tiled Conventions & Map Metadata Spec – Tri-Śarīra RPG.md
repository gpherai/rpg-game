...
- Object-properties:
  ...
  - `spawn_facing` (string, optioneel) – richting waarin de speler kijkt bij spawn. Waarden: `north`, `south`, `east`, `west`. Default: `south`.

**Spawn Facing Principe:** Spawn bij map edge → face INTO the map.

Wanneer een speler via een portal een map betreedt, kijkt deze de richting op die logisch is om de map in te gaan:
- Spawn aan bovenkant map → `spawn_facing: south` (kijk naar beneden, de map in)
- Spawn aan onderkant map → `spawn_facing: north` (kijk naar boven, de map in)
- Spawn aan linkerkant → `spawn_facing: east`
- Spawn aan rechterkant → `spawn_facing: west`
...
Engine gebruikt `spawn_id` om bij zone-wissel te bepalen waar de speler verschijnt, en `spawn_facing` voor de initiële oriëntatie.
